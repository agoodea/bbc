...
window.Whammy = (function() {
        ...
        while (frameNumber < frames.length) {
            ...
            do {
                try {
                    ...
                    frameNumber++;
                    console.log(`1 frameNumber < frames.length && clusterDuration < CLUSTER_MAX_DURATION ${frameNumber < frames.length && clusterDuration < CLUSTER_MAX_DURATION}`);
                } catch (error) {
                    frameNumber++;
                    console.log(`2 frameNumber < frames.length && clusterDuration < CLUSTER_MAX_DURATION ${frameNumber < frames.length && clusterDuration < CLUSTER_MAX_DURATION}`);

                    console.log(`clusterFrames ${error}`);
                }
    ...
    function checkFrames(frames) {
        console.log("object checkFrames");
        console.log(JSON.stringify(frames));
        console.log(JSON.stringify(frames[0]));

        alert("object checkFrames");
        alert(JSON.stringify(frames[0]));
    ...
    WhammyVideo.prototype.encodeFrames = function(callback) {
        ...
        if (this.frames[0].image instanceof ImageData) {
            ...
            var drLength = this.frames.length;
    ...
    WhammyVideo.prototype.compile = function(outputAsArray, callback) {
        ...
        this.encodeFrames(function() {
            ...
            var webm = new toWebM(this.frames.map(
                function(frame) {
                    try {
                        var fris = frame.image.slice(23);
                        console.log(fris);
                        var atb = atob(fris);
                        console.log(atb);
                        var pRiff = parseRIFF(atb);
                        console.log(pRiff);
                        var webp = parseWebP(pRiff);
                        console.log(webp);
